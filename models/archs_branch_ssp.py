...
class MultiTask_Classifier_Head(nn.Module):
    def __init__(self, in_channels=256, reduced_channels=128, num_classes=2):
        ...
        self.ca = CoordAtt(reduced_channels, reduced_channels, reduction=16)
    ...
    def forward(self, x):
        # x: [Batch, 256, 32, 32]
        x = self.reducer(x) # -> [Batch, 128, 32, 32]
        x = self.ca(x)      # -> [Batch, 128, 32, 32] (Attention 적용됨)
        x = self.spp(x)     # -> [Batch, 10368] (Flatten 완료)
...
class UKANClsSSP(nn.Module):
    ...
    def forward(self, x):
        ...
        out = out.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
        cls_out = self.clssfn_head(out)  # CNN 기반 분류 헤드 사용
...
if __name__ == "__main__":
    ...
    # 2. 모델 초기화 및 장치로 이동
    model = UKANClsSSP(num_classes=2, input_channels=1, img_size=1024, patch_size=16, in_chans=3,
